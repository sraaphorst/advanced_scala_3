...
class Module12 extends FunSpec with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  final class ObscureMathUtil {
    @tailrec
    def fib(num: Int, next: Long = 1L, acc: Long = 0L): Long =
      num match {
        case 0 => acc
  ...
  object RockPaperScissors extends TailRec[Outcome.Value] {
    def rock(n: Int): TailRec[Outcome.Value] = n match {
      case 0 => done(Outcome.Tie)
      case _ => tailcall(paper(n - 1))
    }

    def paper(n: Int): TailRec[Outcome.Value] = n match {
      case 0 => done(Outcome.Win)
      case _ => tailcall(scissors(n - 1))
    }

    def scissors(n: Int): TailRec[Outcome.Value] = n match {
      case 0 => done(Outcome.Lose)
      case _ => tailcall(rock(n - 1))
    }
  }
